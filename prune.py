...
parser.add_argument('-0', '--output_dir',
    required=True,
...
for model_to_run in ['quaternion']:

    output_directory = os.path.join('open_lth_data', output_dir_name, model_to_run)
    ...
    # Get the model
    #model = QCNN() if model_to_run == 'quaternion' else CNN()
    ...
    for child in model.children():
        ...
        if model_to_run == 'quaternion':
            ...
            if isinstance(child, layers.QConv2d) or isinstance(child, layers.QLinear):
